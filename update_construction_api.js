...
if (!routeCode.includes('getConstructionFinancialSummary')) {
  routeCode = `const { getConstructionFinancialSummary } = require('../utils/constructionValidator');\n` + routeCode;
}

if (!routeCode.includes('/construction-summary')) {
  const summaryRoute = `
...
`;
  routeCode = routeCode.replace('module.exports = router;', summaryRoute + '\nmodule.exports = router;');
}

const approveRegex = /const validation = await getProjectBudgetValidation\(req\.params\.id, tenantId\);([\s\S]*?if \(validation\.status === 'exceeded' && req\.body\.force !== true\) \{[\s\S]*?return fail\(res, 'BAD_REQUEST', 'Budget exceeded\. Approval blocked\.', 400\);\s*\})/;
const newApprove = `const validation = await getProjectBudgetValidation(req.params.id, tenantId);$1
    
    // Construction Tax & Retention Validation Hard-Block
    const constSummary = await getConstructionFinancialSummary(req.params.id, tenantId);
    const hasFatalError = constSummary.validationFlags.some(f => f.type === 'error');
    if (hasFatalError && req.body.force !== true) {
      return fail(res, 'BAD_REQUEST', 'Construction Compliance Failure: GST, TDS, or Retention rules violated.', 400);
    }
`;
if(routeCode.includes('getConstructionFinancialSummary(req.params.id') === false) {
    routeCode = routeCode.replace(approveRegex, newApprove);
}


fs.writeFileSync(routeFile, routeCode);
console.log('Backend construction validator patched');
