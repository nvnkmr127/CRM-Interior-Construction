...
const utilFile = path.join(__dirname, 'server/src/utils/riskAnalyzer.js');
const utilCode = `
...
if (!routeCode.includes('analyzeFinancialRisk')) {
  routeCode = `const { analyzeFinancialRisk } = require('../utils/riskAnalyzer');\n` + routeCode;
}

if (!routeCode.includes('/risk-analysis')) {
  const riskRoute = `
...
`;
  routeCode = routeCode.replace('module.exports = router;', riskRoute + '\nmodule.exports = router;');
}

fs.writeFileSync(routeFile, routeCode);
console.log('Backend risk analyzer patched');
