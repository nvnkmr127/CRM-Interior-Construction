...
class Pool {
  ...
  async query(sql, params) {
    ...
    if (sql.includes('FROM users WHERE tenant_id') || sql.includes('FROM users WHERE id') || sql.includes('users u')) {
      return { rows: [{ id: 'mock-user-id', tenant_id: 'mock-tenant-id', role_id: 'mock-role-id', name: 'Demo User', email: 'admin@demo.com', status: 'active', password_hash: require('bcryptjs').hashSync('Admin@123', 10), avatar_url: null, created_at: new Date() }] };
    }
