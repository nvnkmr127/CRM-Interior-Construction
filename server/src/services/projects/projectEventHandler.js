...
eventBus.on('lead.stage_changed', async ({ tenantId, userId, lead, _oldStage, newStage }) => {
  try {
    ...
    if (newStage && triggerStages.some(trigger => newStage.name.toLowerCase().includes(trigger))) {
      logger.info(`[Project Event Handler] Lead ${lead.id} moved to ${newStage.name}. Auto-creating project...`);
      ...
      logger.info(`[Project Event Handler] Execution Project successfully created for Lead ${lead.id}`);
    }
  } catch (error) {
    logger.error('[Project Event Handler] Error processing lead.stage_changed:', error);
  }
});
...
eventBus.on('project.milestone_overdue', async ({ tenantId, milestone }) => {
  try {
    ...
    logger.error('[Project Event Handler] Error processing project.milestone_overdue:', error);
  }
});
...
eventBus.on('project.task_overdue', async ({ tenantId, task }) => {
  try {
    ...
    logger.error('[Project Event Handler] Error processing project.task_overdue:', error);
  }
});
...
eventBus.on('project.handover_signed', async ({ tenantId, projectId }) => {
  try {
    ...
    logger.info(`[Project Event Handler] Auto-activated installation warranty and retention schedules for project ${projectId}`);
  } catch (error) {
    logger.error('[Project Event Handler] Error processing project.handover_signed:', error);
  }
});
