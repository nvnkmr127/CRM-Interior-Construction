...
export default function RemoveWidgetModal({ isOpen, onClose, layout, onRemoveWidget }) {
  // Find widgets that are currently in the layout
  const activeIds = new Set(layout.map(l => l.i));
  const activeWidgets = AVAILABLE_WIDGETS.filter(w => activeIds.has(w.id));

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Remove Widgets" size="md">
      <div style={{ marginBottom: '16px', color: 'var(--color-text-secondary)', fontSize: '14px' }}>
        Select a widget to remove it from your dashboard.
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '500px', overflowY: 'auto', paddingRight: '8px' }}>
        {activeWidgets.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px', color: 'var(--color-text-secondary)' }}>
            No widgets left on your dashboard!
          </div>
        ) : (
          activeWidgets.map(w => (
            <div key={w.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', background: 'var(--color-surface-2)', borderRadius: '8px', border: '1px solid var(--color-border)' }}>
              ...
              <Button size="sm" variant="danger" onClick={() => onRemoveWidget(w.id)}>Remove</Button>
            </div>
          ))
        )}
      </div>
